...
For your convenience, add $DFAST_APP_ROOT in your environmental variable `PATH`.
```
...
`locus_tag_prefix` is required if you want your genome to be submitted to the INSDC. DFAST generates DDBJ and GenBank submission files. For more information, please refer to [INSDC submission](docs/insdc_submission.md).
 ...
 If you set `--references` option, OrthoSearch (orthologous gene assignment) is enabled, 
which conducts all-against-all protein alignments between given reference genomes to infer orthologous genes. `--aligner blastp` will let DFAST use BLASTP for protein alignments instead of default GHOSTX.
